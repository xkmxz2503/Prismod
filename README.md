...
- 模组内置滤镜也使用同一套 v1 清单：完整默认包位于 `src/main/resources/assets/prismod/custom/prismod_default_filters/`，其中包含包内 `prismod.pack.json`、`assets/prismod/filters/<id>/` 和运行时辅助 shader。它不是散落在模组根资源中的特殊路径。
